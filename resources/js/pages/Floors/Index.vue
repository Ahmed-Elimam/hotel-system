              ...
              <TableBody>
                <TableRow v-for="row in table.getRowModel().rows" :key="row.original.id">
                  ...
                  <TableCell class="space-x-2">
                    <!-- Update Button -->
                    <Button
                      :disabled="!userRoles.includes('admin') && page.props.user.id !== row.original.creator_id"
                      class="bg-yellow-500 text-black hover:bg-yellow-600 m-2 inline-flex items-center justify-center"
                    >
                      <Link :href="route('floors.edit', row.original.id)" class="w-full h-full flex items-center justify-center">
                        Update
                      </Link>
                    </Button>
